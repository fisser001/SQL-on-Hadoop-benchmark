...
do 
	...
	echo "--------------------------------------------RUN-"$i"---------------------------------------------------------------">><$results_path>
 	...
 	beeline -u 'jdbc:hive2://<$server:$port>/<$path> hive hive' -e "select o_orderpriority, count (distinct o_orderkey) as order_count from paper.lineitem_1 where o_orderdate >= date '1993-07-01' and o_orderdate < date '1993-07-01' + interval '3' month and l_commitdate < l_receiptdate group by o_orderpriority order by o_orderpriority;"&>><$results_path>
...
do 
	...
	echo "--------------------------------------------RUN-"$i"---------------------------------------------------------------">><$results_path>
 	...
 	beeline -u 'jdbc:hive2://<$server:$port>/<$path> hive hive' -e "select sum(l_extendedprice * (1 - l_discount)) as revenue, s_n_name, c_n_name,  year(l_shipdate) as l_year from paper.lineitem_1 where ((s_n_name = 'FRANCE' and c_n_name = 'GERMANY') or (s_n_name = 'GERMANY' and c_n_name = 'FRANCE')) and l_shipdate between date '1995-01-01' and date '1996-12-31' group by s_n_name, c_n_name, year(l_shipdate) order by s_n_name, c_n_name, l_year;"&>><$results_path>
...
do 
	...
	echo "--------------------------------------------RUN-"$i"---------------------------------------------------------------">><$results_path>
 	...
 	beeline -u 'jdbc:hive2://<$server:$port>/<$path> hive hive' -e "select o_year, sum(case when nation = 'BRAZIL' then volume else 0 end) / sum(volume) as mkt_share from ( select year(o_orderdate) as o_year, l_extendedprice * (1-l_discount) as volume, s_n_name as nation from paper.lineitem_1 where c_r_name = 'AMERICA' and o_orderdate between date '1995-01-01' and date '1996-12-31' and p_type = 'ECONOMY ANODIZED STEEL') as all_nations group by o_year order by  o_year;"&>><$results_path>
...
do 
	...
	echo "--------------------------------------------RUN-"$i"---------------------------------------------------------------">><$results_path>
 	...
 	beeline -u 'jdbc:hive2://<$server:$port>/<$path> hive hive' -e "select c_custkey, c_name, sum(l_extendedprice * (1 - l_discount)) as revenue, c_acctbal, c_n_name, c_address, c_phone, c_comment from paper.lineitem_1 where o_orderdate >= date '1993-10-01' and o_orderdate < date '1993-10-01' + interval '3' month and l_returnflag = 'R' group by c_custkey, c_name, c_acctbal, c_phone, c_n_name, c_address, c_comment order by revenue desc limit 20;"&>><$results_path>
...
do 
	...
	echo "--------------------------------------------RUN-"$i"---------------------------------------------------------------">><$results_path>
 	...
 	beeline -u 'jdbc:hive2://<$server:$port>/<$path> hive hive' -e "select c_name, c_custkey, o_orderkey, o_orderdate, o_totalprice, sum(l_quantity) from paper.lineitem_1 group by c_name, c_custkey, o_orderkey, o_orderdate, o_totalprice having sum(l_quantity) > 300 order by o_totalprice desc, o_orderdate;"&>><$results_path>
